...
bool procesarInformacion(char *info, int *encontrado){
   ...
   *encontrado = 0;                                //Si encontramos un comando distinto al requerido?
   ...
   if( (indice = strstr(info, "CWMID")) != NULL){
   ...
   else if( (indice = strstr(info, "LWMDID")) != NULL){
     *encontrado = LWMID;
     frame.createFrame(ASCII, waspmote_id);
     return true;
   }
   else if( (indice = strstr(info, "CHORA")) != NULL){
     
     *encontrado = CHORA;
     
     //CHORA14:01:21:07:13:45:21\0
     indice += 5;
